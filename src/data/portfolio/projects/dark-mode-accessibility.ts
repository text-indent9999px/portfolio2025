...
export const darkModeAccessibility: Project = {
  ...
  summary:
    'BroadcastChannel로 여러 탭의 테마를 실시간 동기화하고, 명도 대비 4.5:1 이상의 토큰으로 접근성을 지킨 테마 시스템입니다.',
  category: 'Accessibility',
  status: 'live',
  role: '1인 개발 (설계 · 구현 · 검증)',
  stack: ['React', 'TypeScript', 'Tailwind CSS', 'CSS Variables', 'BroadcastChannel'],
  ...
  background: [
    ...
    '또 사용자가 브라우저 탭을 여러 개 열어 두면 탭마다 테마가 달라 경험이 파편화되는 문제도 발견했습니다. WCAG 가이드라인을 지키면서 BroadcastChannel로 탭 간 테마를 실시간 동기화하는 테마 시스템을 설계한 이유입니다.',
  ],
  features: [
    '싱글톤 ThemeDetector 설계: 초기화, 구독(subscribe), set/toggle API 제공',
    'useThemeDetector 훅으로 컴포넌트 친화적인 인터페이스 제공',
    'localStorage 값 우선, 시스템 prefers-color-scheme 폴백으로 초기 테마 결정',
    'BroadcastChannel 기반 탭 간 실시간 테마 동기화 (storage 이벤트를 폴백으로 사용)',
  ...
  challenges: [
    ...
    },
    {
      ...
      solution:
        'BroadcastChannel로 테마 변경 이벤트를 브로드캐스트해 모든 탭이 같은 상태를 유지하게 했습니다.',
    },
    {
      problem: '테마 전환 시 HTML 플래시(깜빡임) 발생',
      solution:
        'theme-ready 클래스와 html.dataset.visibility 플래그로 CSS 로드 타이밍을 조율해 깜빡임을 막았습니다.',
    },
    {
